           ...
           .executes(context -> runAutoBattle(context.getSource()))
                   .then(Commands.literal("set")
                           .then(Commands.argument("player", EntityArgument.player())
                                   .then(Commands.argument("off", BoolArgumentType.bool())
                                           .executes(context -> setAutoBattle(context.getSource(), EntityArgument.getPlayer(context, "player"), false, false))
                                   )
                                   .then(Commands.argument("on", BoolArgumentType.bool())
                                           .then(Commands.argument("forceOut", BoolArgumentType.bool())
                                                   .executes(context -> setAutoBattle(context.getSource(), EntityArgument.getPlayer(context, "player"), true, BoolArgumentType.getBool(context, "forceOut")))
                                           )
                                   )
                           )
           );
